...
namespace PRSC_Player_Auction_System
{
    ...
    //  CONSTANTS
    // ════════════════════════════════════════════════════════════════════
    internal static class AuctionDefaults
    {
        public const decimal DefaultFund = 20_000m;   // ← single place to change the default
    }
    ...
    public class TeamReportForm : Form
    {
                              ...
                              bool isTeamA)
        {
            ...
            _isTeamA = isTeamA;
            _accent = isTeamA ? Color.FromArgb(50, 205, 50) : Color.FromArgb(100, 149, 237);
            _panelBg = isTeamA ? Color.FromArgb(0, 40, 0) : Color.FromArgb(0, 0, 40);
        ...
        private void BuildUI()
        {
            ...
            var div1 = new Panel { Dock = DockStyle.Top, Height = 2, BackColor = _accent };
            ...
            void MkStat(string lbl, string val, Color valColor, int x)
            {
                var p = new Panel
                {
            ...
            MkStat("PLAYERS BOUGHT", _bought.Count.ToString(), Color.FromArgb(220, 220, 220), 10);
            MkStat("TOTAL SPENT", $"৳ {spent:N0}", Color.FromArgb(255, 80, 80), 220);
            MkStat("REMAINING FUND", $"৳ {_remainingFund:N0}", Color.Gold, 440);

            var div2 = new Panel { Dock = DockStyle.Top, Height = 1, BackColor = Color.FromArgb(40, 40, 40) };
            ...
            // FIX: Use ClientSize.Width from InitializeComponent (700) so position is correct
            var btnExport = MkBtn("📄  Export PDF", Color.FromArgb(160, 80, 0), 10);
            var btnClose = MkBtn("✖  Close", Color.FromArgb(80, 20, 20), 700 - 178);
            ...
            dgv.AlternatingRowsDefaultCellStyle = new DataGridViewCellStyle(cellStyle) { BackColor = _altBg };
        ...
        private void BuildTeamPdf(string path)
        {
            ...
            tSum.AddCell(SumCell("PLAYERS BOUGHT", _bought.Count.ToString(), F(16f, iTextSharp.text.Font.BOLD, cLight)));
            tSum.AddCell(SumCell("TOTAL SPENT", $"BDT {spent:N0}", F(13f, iTextSharp.text.Font.BOLD, cRed)));
            tSum.AddCell(SumCell("REMAINING FUND", $"BDT {_remainingFund:N0}", F(13f, iTextSharp.text.Font.BOLD, cGold)));
            doc.Add(tSum);
    ...
    public partial class MainForm : Form
    {
        ...
        public decimal TeamAFund
        {
            get
            {
                if (txtTeamAFund == null) return AuctionDefaults.DefaultFund;
        ...
        public decimal TeamBFund
        {
            get
            {
                if (txtTeamBFund == null) return AuctionDefaults.DefaultFund;
        ...
        public string TeamAName => txtTeamAName?.Text?.Trim() ?? "Team Alpha";
        public string TeamBName => txtTeamBName?.Text?.Trim() ?? "Team Beta";
        ...
        private void LoadFromDatabase()
        {
            ...
            // Load team funds — FIX: fall back to DefaultFund (20,000), not 100,000
            try
            {
            ...
            {
                if (txtTeamAFund != null) txtTeamAFund.Text = AuctionDefaults.DefaultFund.ToString("N0");
                if (txtTeamBFund != null) txtTeamBFund.Text = AuctionDefaults.DefaultFund.ToString("N0");
            }
        ...
        // ── Team report buttons ───────────────────────────────────────────
        private void btnTeamAInfo_Click(object sender, EventArgs e) => ShowTeamReport(isTeamA: true);
        private void btnTeamBInfo_Click(object sender, EventArgs e) => ShowTeamReport(isTeamA: false);
        ...
        private void ShowTeamReport(bool isTeamA)
        {
            ...
            decimal initialFund = fund + spent;   // reconstruct what it was before spending
        ...
        {
            using (var dlg = new AddPlayerForm(teamNames: GetTeamNames()))
            {
        ...
        private void btnEditPlayer_Click(object sender, EventArgs e)
        {
            ...
            using (var dlg = new AddPlayerForm(player, GetTeamNames()))
            {
                if (dlg.ShowDialog() == DialogResult.OK)
                {
                    var ed = dlg.Result;
        ...
        private void btnReset_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show(
                    ...
                    $"• Reset both team funds to ৳ {AuctionDefaults.DefaultFund:N0}\n\n" +
            ...
            // 2. FIX: Reset team funds to default in DB AND in UI
            try
            {
                DatabaseHelper.UpdateTeamFund("TeamA", AuctionDefaults.DefaultFund);
                DatabaseHelper.UpdateTeamFund("TeamB", AuctionDefaults.DefaultFund);
            }
            catch { }

            // 3. Update UI text boxes directly (in case DB call above threw)
            if (txtTeamAFund != null) txtTeamAFund.Text = AuctionDefaults.DefaultFund.ToString("N0");
            if (txtTeamBFund != null) txtTeamBFund.Text = AuctionDefaults.DefaultFund.ToString("N0");
            ...
            if (lblStatusBar != null)
                lblStatusBar.Text = $"  Auction reset. Both funds restored to ৳ {AuctionDefaults.DefaultFund:N0}. Ready to begin.";
        }
        ...
        private void ExportToPdf(string filePath)
        {
            ...
            doc.Add(new Paragraph("PRSC PLAYER AUCTION SYSTEM",
                MkFont(22f, iTextSharp.text.Font.BOLD, cGreen))
            { Alignment = Element.ALIGN_CENTER, SpacingAfter = 2f });

            doc.Add(new Paragraph(
                $"Generated: {DateTime.Now:dd MMMM yyyy   HH:mm}\nDeveloped by Emon Joy",
                MkFont(7f, iTextSharp.text.Font.NORMAL, cDim))
            { Alignment = Element.ALIGN_CENTER, SpacingAfter = 8f });

            // Header table (Team A | Team B | Stats)
            ...
            tblA.AddCell(TR("TEAM A", MkFont(7.5f, iTextSharp.text.Font.NORMAL, cDim), cTeamABg, pt: 8f, pb: 2f));
            tblA.AddCell(TR(teamAName, MkFont(14f, iTextSharp.text.Font.BOLD, cGreen), cTeamABg, pt: 0f, pb: 4f));
            tblA.AddCell(TR($"Fund: BDT {teamAFund:N0}", MkFont(10f, iTextSharp.text.Font.BOLD, cGold), cTeamABg, pt: 2f, pb: 2f));
            tblA.AddCell(TR($"Players: {cntA}", MkFont(7.5f, iTextSharp.text.Font.NORMAL, cDim), cTeamABg, pt: 2f, pb: 8f));
            ...
            tblB.AddCell(TR("TEAM B", MkFont(7.5f, iTextSharp.text.Font.NORMAL, cDim), cTeamBBg, pt: 8f, pb: 2f));
            tblB.AddCell(TR(teamBName, MkFont(14f, iTextSharp.text.Font.BOLD, cBlue), cTeamBBg, pt: 0f, pb: 4f));
            tblB.AddCell(TR($"Fund: BDT {teamBFund:N0}", MkFont(10f, iTextSharp.text.Font.BOLD, cGold), cTeamBBg, pt: 2f, pb: 2f));
            tblB.AddCell(TR($"Players: {cntB}", MkFont(7.5f, iTextSharp.text.Font.NORMAL, cDim), cTeamBBg, pt: 2f, pb: 8f));
            ...
            {
                var inner = new PdfPTable(1) { WidthPercentage = 100f };
                inner.AddCell(new PdfPCell(new Phrase(lbl, MkFont(7.5f, iTextSharp.text.Font.NORMAL, cDim)))
                {
            ...
            tblStats.AddCell(SR("TOTAL", total.ToString(), MkFont(12f, iTextSharp.text.Font.BOLD, cGold)));
            tblStats.AddCell(SR("SOLD", sold.ToString(), MkFont(12f, iTextSharp.text.Font.BOLD, cRed)));
            tblStats.AddCell(SR("FREE", avail.ToString(), MkFont(12f, iTextSharp.text.Font.BOLD, cGreen)));
            ...
            foreach (var h in new[] { "#", "Player Name", "Position", "Skill",
                ...
                tbl.AddCell(new PdfPCell(new Phrase(h, MkFont(9f, iTextSharp.text.Font.BOLD, cGreen)))
                {
            ...
            foreach (var p in snapshot)
            {
                ...
                bool isS = p.IsSold;
                var cf = isS ? MkFont(8.5f, iTextSharp.text.Font.NORMAL, cDim)
                               : MkFont(8.5f, iTextSharp.text.Font.NORMAL, cLight);
                ...
                tbl.AddCell(MC(p.AssignedTeam ?? "",
                    isS ? MkFont(8.5f, iTextSharp.text.Font.NORMAL, cBlue) : cf, bg));
                tbl.AddCell(MC(p.Status ?? "",
                    isS ? MkFont(8.5f, iTextSharp.text.Font.BOLD, cRed)
                        : MkFont(8.5f, iTextSharp.text.Font.BOLD, cGreen), bg));
        ...
        // ── Helpers ───────────────────────────────────────────────────────
        // FIX: Use Tag (bound Player object) instead of row index, which breaks with sorting
        private Player GetSelectedPlayer()
        {
            if (dgvPlayers == null || dgvPlayers.SelectedRows.Count == 0) return null;
            var row = dgvPlayers.SelectedRows[0];
            // DataGridView bound to BindingSource — get item directly via DataBoundItem
            if (row.DataBoundItem is Player p) return p;
            // Fallback: use index
            int idx = row.Index;
